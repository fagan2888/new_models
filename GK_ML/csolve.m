...
%------------ verbose ----------------
frequencyPrint = 500;  
verbose=1;% if this is set to zero, all screen output is suppressed if
% set to 1 each iteration output is printed, if 2 frequencyPrint determines 
% the frequency informations are printed out
...
while ~done
   ...
   elseif (verbose == 2 & mod(itct,frequencyPrint) == 0)
      fprintf(1,'\nitct %d, af %g, lambda %g, rc %g',itct,afmin,lambdamin,rc)
      fprintf(1,'\n   x  %10g %10g %10g %10g',xmin);
      fprintf(1,'\n   f  %10g %10g %10g %10g',fmin);
   end
