...
// Change:
// - baseline modell results should be the same as with gammaEZ = 2 and psiEZ = 0.5
// - exchange utility fucntion Household
// - taking out the Epstein Zin utility
// household utility c^nuEZ (1-L)^(1-nuEZ)


parameters betta sig hh varphi zetta theta alfa G_over_Y eta_i epsilon gam gam_P kappa_pi kappa_y rho_i rho_ksi sigma_ksi rho_a sigma_a rho_g sigma_g sigma_Ne sigma_i rho_shock_psi sigma_psi kappa tau omega lambda chi b delta_c G_ss I_ss nuEZ;
var Y Ym K  L I C G Q varrho Lambda Rk R N Ne Nn nu eta phi z x Pm U X D F Z i delta In a ksi g infl inflstar ;
...
model;
exp(Pm)*(1-alfa)*exp(Y)/exp(L) = exp(C)/(1-exp(L)) *(1-nuEZ)/nuEZ ;

exp(varrho) = nuEZ*exp(C)^(nuEZ-1)*(1-exp(L))^(1-nuEZ);
...
initval;
C = log(0.451610879111626);
D = log(1.000000000000003);
F = log(2.477215009286162);
G = log(0.169757100000000);
I = log(0.124352685734627);
In = 0; 
K = log(4.974107642854897);
L = log(0.292857916980712);
N = log(1.243526844272011);
Ne = log(1.232445626747412);
...
Ym = log(0.745720664846253);
Z = log(3.259411096840997);
delta = log(0.024999998927095);
eta = log(1.511020932460977);
i = log(1.010101010101010);
infl = log(1.000000000000000);
inflstar = log(1.000000000000000);
nu = log(0.003739777068239);
phi = log(4.000000213720237);
...
stoch_simul(PRUNING, order=2, periods=2000, irf=40, nographs);
